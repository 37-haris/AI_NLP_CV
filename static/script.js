...
openBtn.addEventListener('click',   () => { sidebar.classList.remove('collapsed'); openBtn.style.display = 'none'; });
...
async function loadChatList() {
  ...
  chatList.innerHTML = '';
  chats.forEach(chat => addChatItemToSidebar(chat.id, chat.title, false));
}

function addChatItemToSidebar(chatId, title, activate = true) {
  if (activate) chatList.querySelectorAll('.chat-item').forEach(el => el.classList.remove('active'));
  const item = document.createElement('div');
  item.className = 'chat-item' + (activate ? ' active' : '');
  ...
  item.addEventListener('click', () => openChat(chatId, item));
...
async function openChat(chatId, itemEl) {
  ...
  topbarTitle.textContent = itemEl.querySelector('span').textContent;
  ...
  messages.forEach(msg => {
    if (msg.role === 'user') {
      // Build image object from saved path if available
      const imgObj = msg.image_path
        ? { dataUrl: msg.image_path, name: msg.image_path.split('/').pop() }
        : null;
      appendUserMessage(msg.content, imgObj);
    } else {
      ...
      const bubble = document.createElement('div'); bubble.className = 'bubble ai'; bubble.style.width = '100%';
      body.appendChild(name); body.appendChild(bubble);
      row.appendChild(av); row.appendChild(body);
      messagesEl.appendChild(row);
      renderLangCards(bubble, data.en, data.fr);
    }
  });
  scrollBottom();
}
...
mainEl.addEventListener('dragover',  e => { e.preventDefault(); dragOverlay.classList.add('active'); });
mainEl.addEventListener('dragleave', e => { if (!mainEl.contains(e.relatedTarget)) dragOverlay.classList.remove('active'); });
...
function showImageStrip() {
  ...
  const img  = document.createElement('img');  img.src = pendingImage.dataUrl; img.className = 'img-thumb'; img.alt = pendingImage.name;
  const rm   = document.createElement('button'); rm.className = 'img-remove'; rm.title = 'Remove';
  ...
  meta.innerHTML = `<span class="img-filename">${pendingImage.name}</span><span class="img-filesize">${pendingImage.size}</span>`;
  imgStrip.appendChild(wrap); imgStrip.appendChild(meta);
  imgStrip.classList.add('has-image'); imgStrip.style.padding = '10px 14px 0';
}

function clearImage() {
  pendingImage = null; pendingFile = null;
  imgStrip.innerHTML = ''; imgStrip.classList.remove('has-image'); updateSendBtn();
}
...
async function send() {
  ...
  if (!currentChatId) {
    const title   = message || img.name || 'Image analysis';
    const res     = await fetch('/chats/', {
    ...
    addChatItemToSidebar(currentChatId, title, true);
  }
...
function appendUserMessage(text, img) {
  ...
  if (img) {
    ...
    imgEl.src = img.dataUrl;  // works for both blob URLs and /static/uploads/ paths
    imgEl.className = 'img-preview'; imgEl.alt = img.name || '';
    bubble.appendChild(imgEl);
  }
  if (text) {
    const span = document.createElement('span'); span.className = 'bubble-text'; span.textContent = text;
    bubble.appendChild(span);
  }
  body.appendChild(name); body.appendChild(bubble);
  row.appendChild(body); row.appendChild(av);
...
async function callBackend(imageFile, userText) {
  ...
  bubble.appendChild(dots); body.appendChild(name); body.appendChild(bubble);
  row.appendChild(av); row.appendChild(body); messagesEl.appendChild(row); scrollBottom();

  try {
    if (!imageFile) throw new Error('Please upload an image before sending.');

    const formData = new FormData();
    formData.append('image', imageFile);
    formData.append('user_text', userText || '');

    const response = await fetch('/auth/predict', { method: 'POST', body: formData });
    ...
    await fetch(`/chats/${currentChatId}/messages`, {
      method: 'POST', headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        chat_id: currentChatId, role: 'ai',
        content: JSON.stringify({ en: enText, fr: frText })
      })
    });
    ...
    errEl.className = 'bubble-text'; errEl.style.color = '#fc8181';
...
function makeLangCard(lang) {
  ...
  const textEl = document.createElement('span'); textEl.className = 'lang-card-text';
  card.appendChild(label); card.appendChild(spinner); card.appendChild(textEl);
...
function typeIntoCard(textEl, spinnerEl, text, onDone) {
  ...
  let i = 0;
  const step = () => {
    if (i < text.length) { cursor.insertAdjacentText('beforebegin', text[i++]); scrollBottom(); setTimeout(step, 14); }
...
function renderLangCards(bubble, enText, frText) {
  const cards = document.createElement('div'); cards.className = 'lang-cards';
  const en = makeLangCard('en'); const fr = makeLangCard('fr');
  cards.appendChild(en.card); cards.appendChild(fr.card);
  bubble.appendChild(cards); scrollBottom();
  typeIntoCard(en.textEl, en.spinner, enText, function () {
    typeIntoCard(fr.textEl, fr.spinner, frText, function () { isGenerating = false; updateSendBtn(); });
  });
}
...
newChatBtn.addEventListener('click', () => {
  ...
  clearImage(); msgInput.value = ''; msgInput.style.height = 'auto'; updateSendBtn();
  chatList.querySelectorAll('.chat-item').forEach(el => el.classList.remove('active'));
});
